import Header from "../components/Header";

const Credits = () => {


  return (
    <>
      <Header />
      <div id="credit_page">
        <h1>CREDITS</h1>
        <h2>ICONS</h2>
        <ul>
        <li>add.png icon made by Freepik from 
                <a href="http://www.flaticon.com" title="Flaticon"> www.flaticon.com </a>
                is licensed by 
                <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank"> CC 3.0 BY</a>
                . Reference to image link:
                <a href="https://www.flaticon.com/free-icons/plus" title="plus icons">Plus icons created by srip - Flaticon</a>
            </li>
            <li>brush-size.png icon made by Freepik from 
                <a href="http://www.flaticon.com" title="Flaticon"> www.flaticon.com </a>
                is licensed by 
                <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank"> CC 3.0 BY</a>
                . Reference to image link:
                <a href="https://www.flaticon.com/free-icons/graphic-design" title="graphic design icons"> Graphic design icons created by Freepik - Flaticon</a>
            </li>
            <li>default-brush.png icon made by Freepik from 
                <a href="http://www.flaticon.com" title="Flaticon"> www.flaticon.com </a>
                is licensed by 
                <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank"> CC 3.0 BY</a>
                . Reference to image link:
                <a href="https://www.flaticon.com/free-icons/brush" title="brush icons">Brush icons created by Freepik - Flaticon</a>    
            </li>
            <li>eraser.png icon made by Freepik from 
                <a href="http://www.flaticon.com" title="Flaticon"> www.flaticon.com </a>
                is licensed by 
                <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank"> CC 3.0 BY</a>
                . Reference to image link:
                <a href="https://www.flaticon.com/free-icons/eraser" title="eraser icons">Eraser icons created by IYIKON - Flaticon</a>   
            </li>
            <li>eraser.png icon made by Freepik from 
                <a href="http://www.flaticon.com" title="Flaticon"> www.flaticon.com </a>
                is licensed by 
                <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank"> CC 3.0 BY</a>
                . Reference to image link:
                <a href="https://www.flaticon.com/free-icons/eraser" title="eraser icons">Eraser icons created by IYIKON - Flaticon</a>   
            </li>
            <li>gradient-brush.png icon made by Freepik from 
                <a href="http://www.flaticon.com" title="Flaticon"> www.flaticon.com </a>
                is licensed by 
                <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank"> CC 3.0 BY</a>
                . Reference to image link:
                <a href="https://www.flaticon.com/free-icons/brush" title="brush icons">Brush icons created by Freepik - Flaticon</a>
            </li>
            <li>pen.png icon made by Freepik from 
                <a href="http://www.flaticon.com" title="Flaticon"> www.flaticon.com </a>
                is licensed by 
                <a href="http://creativecommons.org/licenses/by/3.0/" title="Creative Commons BY 3.0" target="_blank"> CC 3.0 BY</a>
                . Reference to image link:
                <a href="https://www.flaticon.com/free-icons/pen-drive" title="pen drive icons">Pen drive icons created by Good Ware - Flaticon</a>
            </li>
        </ul>

        <h2>HTML, CSS and Javascript</h2>
        <ul>
            <li>Mostly used <a href="http://stackoverflow.com/">Stackoverflow</a> for code help. References are linked at the location 
            where the code resides, as comments.</li>
            <li>Also used <a href="https://www.w3schools.com/">W3Schools</a> for a lot of coding/css help. References are linked at the location 
            where the code resides, as comments.</li>
        </ul>

        <h2>Images</h2>
        <ul>
        <li>
            Homepage background Image: <a href='https://www.freepik.com/photos/mountain-water'>Mountain water photo created by liuzishan - www.freepik.com</a>
        </li>
           
        </ul>
    </div>
    </>
  );
};

export default Credits;
